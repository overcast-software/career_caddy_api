    ...
    def handle(self, *args, **options):
        ...
        for u in User.objects.order_by("id").only("id", "username", "email"):
            try:
                ...
                # registration-time rule, and installs bootstrapped
                # before CC-123 moved the default off it have a
                # superuser legitimately named `admin` — flagging it here
                # would put a permanent false violation in front of those
                # operators and dull the "OK" signal. This is the ONLY
                # allow_reserved caller; no write path sets it. Pinned by
                # AuditUsernamesCommandTests.test_reserved_username_
                # not_flagged.
