...
def test_returns_none_on_normal_description():
    ...
    assert detect_posting_status(text) is None


def test_detects_no_longer_accepting():
    assert (
        detect_posting_status("This role is no longer accepting applications.")
        == "closed"
    )
...
def test_detects_position_closed():
    assert (
        detect_posting_status("Notice: this position is closed.") == "closed"
    )


def test_detects_position_filled():
    assert (
        detect_posting_status("This position has been filled. Thanks!")
        == "closed"
    )
...
def test_detects_explicit_closed_marker():
    ...
    assert detect_posting_status(text) == "closed"


def test_detects_short_closed_bracket():
    assert detect_posting_status("[ Closed ]\nrest of post") == "closed"


def test_case_insensitive():
    assert (
        detect_posting_status("WE ARE NO LONGER ACCEPTING applications")
        == "closed"
    )


def test_returns_none_on_empty():
    assert detect_posting_status("") is None
    assert detect_posting_status(None) is None
