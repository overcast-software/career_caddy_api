...
drift attaches itself to whoever migrates next, which is how the diff 0138
finally landed survived so long — the hand-written `Meta.indexes` names in
0088/0098/0108 and the `models.AutoField` spellings in 0087/0088/0098 had been
regenerating ever since, and later authors (0136, CC-213) kept hand-stripping
them back out.

Note the `interactive=False` on that `call_command`. It is load-bearing:
`ask_rename`/`ask_rename_model` have no `dry_run` guard, so without it a
renamed field or model sends the check into `input()` — `EOFError` under CI,
a silent hang under a TTY, and the prompt invisible either way because it
goes to the captured stdout.

The schema half matters more than it looks. `makemigrations` compares
...
to two `company_id` indexes, and the mismatch only surfaced later when an
autogenerated `RenameIndex` died with `relation ... does not exist` on a real
`migrate`. Two assertions cover it: every declared `Meta.indexes` name must
exist verbatim (those are the names migrations manipulate by name), and every
`db_index` column must have *some* btree leading on it. **When you write
`RunSQL` that touches an index, constraint or column, give it matching
`state_operations` (via `SeparateDatabaseAndState`) or you are re-arming the
same trap.**

What is still broken and not gated: the nanoid PK swaps (0119–0124) rebuilt
~45 FK columns in raw SQL with one hand-named `<table>_<column>_idx` apiece,
so the Django-hashed implicit FK index and its `_like` sibling are missing
across most of the schema. Names there are wrong nearly everywhere, which is
why the second assertion checks coverage rather than naming. Repairing it is
its own ticket.
